...
La configuración correspondiente a \(T=0.5\) emerge de forma inequívoca como el punto óptimo de funcionamiento del sistema. Esta temperatura registra simultáneamente la mayor similitud media global (94.02\,\%) y la menor desviación estándar observada durante la experimentación (\(\sigma = 5.06\)). La convergencia de ambas métricas estadísticas en un mismo valor paramétrico constituye una evidencia sólida de que una cantidad moderada de aleatoriedad maximiza el rendimiento global del proceso de digitalización estructurada.
...
La diferencia se aprecia claramente al comparar los resultados obtenidos bajo determinismo estricto con aquellos alcanzados dentro de la ventana térmica comprendida entre \(T=0.1\) y \(T=0.5\), identificada durante este trabajo como el \textit{Sweet Spot} del sistema.
...
Como consecuencia, pequeños errores de alineación, saltos de línea anómalos o ambigüedades visuales pueden propagarse sin posibilidad de corrección. Por el contrario, una temperatura moderada, comprendida aproximadamente entre \(T=0.25\) y \(T=0.5\), introduce la flexibilidad necesaria para explorar interpretaciones alternativas de la estructura documental sin comprometer la coherencia global ni vulnerar las restricciones sintácticas impuestas por la gramática XML.
...
A la luz de esta evidencia, la ventana térmica comprendida entre \(T=0.25\) y \(T=0.5\) se consolida como la configuración más equilibrada para un sistema generalista de digitalización de gramáticas de la ALMG. Este intervalo proporciona suficiente flexibilidad para mejorar la extracción en los documentos más complejos, manteniendo simultáneamente la estabilidad necesaria para evitar degradaciones en aquellas páginas que presentan una estructura clara y fácilmente interpretable.
