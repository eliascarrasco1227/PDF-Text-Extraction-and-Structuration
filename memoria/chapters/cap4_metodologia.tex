...
\begin{itemize}
    ...
    \item \textbf{Fase de ingeniería de prompts y estructuración (Few-Shot):} Se implementó la lógica de \textit{Few-Shot Learning} (visible en la carpeta \texttt{data/few-shot}), proporcionando al modelo ejemplos de XML validado para guiar la generación. Metodológicamente, se optó por la técnica avanzada de pre-carga de historial (\textit{History Pre-loading}) a nivel de SDK, inyectando los pares de imagen y XML directamente en el búfer de la sesión antes del envío del mensaje del usuario, lo que optimizó drásticamente la latencia y la eficiencia en la transferencia de red. Durante esta fase se documentó el fenómeno de \textit{overfitting} o pérdida de generalización cognitiva al incrustar capturas fijas directamente en el prompt; el modelo tendía a reproducir de forma literal metadatos espurios del ejemplo (como la cadena ``Adidas'' o las oraciones exactas de la muestra) en las nuevas páginas, lo que obligó a refinar la abstracción del prompt textual. En esta fase se definió el DTD inicial.
...
\begin{itemize}
    \item \textbf{\texttt{google-generativeai}:} Librería principal para la comunicación con la API de Gemini (modelos 1.5 Pro, Flash y 2.5 Flash-lite). Gestiona el envío de imágenes y la recepción de respuestas estructuradas.
\end{itemize}

\paragraph{Procesamiento de documentos y visión}

\begin{itemize}
    \item \textbf{\texttt{pdf2image}:} Utilizada para convertir las páginas del PDF en imágenes de alta resolución, paso necesario para que el modelo multimodal pueda ``ver'' la disposición tipográfica de las gramáticas.
    
    \item \textbf{\texttt{Pillow (PIL)}:} Empleada para el preprocesamiento de imágenes antes de su envío a la API.
\end{itemize}
...
La validez de este estudio depende de la representatividad y complejidad del material documental seleccionado. Para ello, se ha conformado un corpus de trabajo compuesto por tres gramáticas normativas fundamentales, editadas por la Academia de Lenguas Mayas de Guatemala (ALMG), que representan diferentes variantes y desafíos técnicos.
...
\begin{itemize}
    \item \textbf{Relevancia demográfica:} El K'iche', el Kaqchikel y el Mam se encuentran entre las lenguas mayas con mayor número de hablantes y producción académica, lo que garantiza que los resultados tengan un impacto social significativo.
...
\begin{itemize}
    ...
    \item \textbf{Gramática Normativa Mam:} Incorporada para asegurar que el pipeline sea capaz de manejar variaciones en el conjunto de caracteres Unicode y estructuras de numeración jerárquica que difieren levemente de las otras dos variantes.
\end{itemize}
