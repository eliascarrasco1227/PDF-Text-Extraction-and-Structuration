...
\begin{enumerate}
    \item \textbf{Capa de ingesta y preprocesamiento:} El sistema localiza los archivos PDF en el directorio \texttt{data/} y extrae el rango de páginas solicitado. En la ruta experimental principal, esta extracción se realiza mediante segmentación del PDF en un flujo binario (\textit{bytes}), sin rasterizar las páginas.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{config/}:} Centraliza los parámetros del sistema, rutas de directorios y la gestión de la arquitectura técnica. Algunos ejemplos de configuración incluyen la selección del modelo de IA, la temperatura de generación, los directorios de entrada y salida, el rango de páginas a procesar, rutas a los archivos de few-shot y la ubicación de la DTD...
    
    \item \textbf{\texttt{validator/}:} Aloja las reglas formales (DTD) y el script de validación que garantiza la calidad de los datos.
    
    \item \textbf{\texttt{latex\_generator/}:} Módulo especializado en la conversión de etiquetas XML a comandos de tipografía científica (\LaTeX{}), incluyendo la gestión de caracteres especiales y glosas interlineales.
...
El primer eslabón del \textit{pipeline} es el módulo encargado de la ingesta y manipulación de los archivos fuente. Dado que las gramáticas de la ALMG presentan una naturaleza heterogénea, se ha implementado una estrategia principal de segmentación y extracción de flujo binario que permite aislar fragmentos del documento original sin alterar su estructura interna.
...
A través del módulo \texttt{prompt\_reader.py}, el sistema incorpora a la petición ejemplos reales formados por pares de entrada y salida compuestos por una página del pdf original y su correspondiente XML validado, almacenados en la carpeta \texttt{data/few-shot}.
...
Una de las características más relevantes de este módulo es su sistema de almacenamiento jerárquico de resultados. Cada ejecución se organiza automáticamente según criterios como el idioma procesado y wl rango de páginas utilizadas.
