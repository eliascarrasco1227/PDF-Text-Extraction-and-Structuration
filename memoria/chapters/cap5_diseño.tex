...
\begin{enumerate}
    \item \textbf{Capa de ingesta y preprocesamiento:} El sistema localiza los archivos PDF en el directorio \texttt{data/} y los transforma en imágenes de alta resolución compatibles con la entrada multimodal de la API.
    
    \item \textbf{Capa de inferencia (Motor de IA):} Se envían las imágenes junto con un \textit{prompt} especializado y ejemplos de entrenamiento (\textit{few-shot}) para obtener una respuesta en formato XML.
...
Un factor crítico en este desarrollo es la preservación de la integridad del documento original durante el filtrado. Al trabajar con la manipulación de \textit{bytes} en lugar de conversiones a imagen, el sistema garantiza:
...
Este módulo constituye el núcleo cognitivo del sistema. Su función es orquestar la comunicación con los Modelos de Lenguaje de Gran Tamaño (LLMs) de la familia Gemini, transformando las imágenes procesadas en datos estructurados mediante técnicas avanzadas de ingeniería de \textit{prompts}.
...
El módulo está diseñado para realizar un barrido (\textit{sweep}) sobre el parámetro de temperatura del modelo. Para ello, el orquestador ejecuta el \textit{pipeline} completo de forma iterativa, variando la temperatura en un conjunto predefinido de valores, como por ejemplo 0.0, 0.1, 0.25, 0.5, 1.0 y 1.5.
...
\begin{enumerate}
    \item Obtención del documento de prueba mediante \texttt{pdf\_processor.py}.
...
El sistema incorpora además un mecanismo de versionado incremental. Cada resultado generado se almacena con un identificador de versión secuencial (\texttt{v1}, \texttt{v2}, \texttt{v3}, etc.), permitiendo estudiar la estabilidad del modelo a lo largo de múltiples ejecuciones.
