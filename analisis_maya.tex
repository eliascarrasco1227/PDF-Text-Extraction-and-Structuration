\documentclass{article}%
\usepackage[T1]{fontenc}%
\usepackage[utf8]{inputenc}%
\usepackage{lmodern}%
\usepackage{textcomp}%
\usepackage{lastpage}%
\usepackage{expex}%
\usepackage[utf8]{inputenc}%
%
\title{Análisis Lingüístico: Gramatica{-}Normativa{-}Kaqchikel.pdf}%
\author{Generado por Maya API TFM}%
%
\begin{document}%
\normalsize%
\maketitle%
\section{Página 170}%
\label{sec:Pgina170}%
plural) del objeto y sujeto. La palabra principal incluye primeramente el tiempo/aspecto\newline%
%
o modo (excepto en el perfectivo que es la última), seguido por el juego absolutivo,\newline%
%
juego ergativo, la raíz o base en calidad de obligatorios y sufijos de categoría en casos\newline%
%
específicos (si el verbo es derivado). Como elementos opcionales incluye el movimiento,\newline%
%
su posición es siempre antes del juego ergativo. Ejemplos:\newline%
%
\subsection{Marca de tiempo/aspecto/modo}%
\label{subsec:Marcadetiempo/aspecto/modo}%

%
Marca de tiempo/aspecto/modo:\newline%
%
\ex
\begingl
\gla Xb'entz'eta' na kan ri ak'wal. //
\glb \textbf{X-} \textbf{Ø-} \textbf{b'e-} \textbf{n-} \textbf{tz'et-} \textbf{a'} \textbf{na} \textbf{kan} \textbf{ri} \textbf{ak'wal} //
\glc COM- B3S- MOV- A1s- ver- SC PAR DIR DET niño //
\gld Xb'entz'eta' na kan ri ak'wal. [\textsc{SVT}] //
\glft `\textit{Todavía fui a ver al niño.}' //
\endgl
\xe
%
\subsection{Marca el objeto en el verbo}%
\label{subsec:Marcaelobjetoenelverbo}%

%
Marca el objeto en el verbo:\newline%
%
\ex
\begingl
\gla ...kan jeb'ël ok xerutz'ët ri ch'uta'q xtani' rija'... //
\glb \textbf{kan} \textbf{jeb'ël} \textbf{ok} \textbf{x-} \textbf{e-} \textbf{ru-} \textbf{tz'ët} \textbf{ri} \textbf{ch'uta'q} \textbf{xtan-} \textbf{i'} \textbf{rija'...} //
\glc PAR hermosa DIR COM- B3p- A3s- ver DET DIM Niña- PL él //
\gld kan jeb'ël ok xerutz'ët ri ch'uta'q xtani' [\textsc{SVT}] //
\glft `\textit{...vio muy hermosas a las niñas...}' //
\endgl
\xe
%
\subsection{Marca el sujeto en el verbo}%
\label{subsec:Marcaelsujetoenelverbo}%

%
Marca el sujeto en el verbo:\newline%
%
\ex
\begingl
\gla Xkilöq' jun uqaj ri ka'i' achi'a'. //
\glb \textbf{X-} \textbf{Ø-} \textbf{ki-} \textbf{löq'} \textbf{jun} \textbf{uqaj} \textbf{ri} \textbf{ka'i'} \textbf{achi'-} \textbf{a'.} //
\glc COM- B3s- A3p- comprar IND corte DET dos hombre- PL //
\gld Xkilöq' jun uqaj [\textsc{SVT}] //
\glft `\textit{Los dos hombres compraron un corte.}' //
\endgl
\xe
%
\ex
\begingl
\gla Xusipaj ri xkoya' ri xtän. //
\glb \textbf{X-} \textbf{Ø-} \textbf{u-} \textbf{sip-} \textbf{a-} \textbf{j} \textbf{ri} \textbf{xkoya'} \textbf{ri} \textbf{xtän.} //
\glc COM- B3s- A3s- regalar- BV- SC DET tomate DET señorita //
\gld Xusipaj ri xkoya' [\textsc{SVT}] //
\glft `\textit{La señorita regaló el tomate.}' //
\endgl
\xe
%
\subsection{Marca movimiento en el verbo}%
\label{subsec:Marcamovimientoenelverbo}%

%
Marca movimiento en el verbo:\newline%
%
\ex
\begingl
\gla ...nb'esamäj qatata', nb'eqato',... //
\glb  //
\glc  //
\gld nb'eqato' [\textsc{SVT}] //
\glft `\textit{...nuestro papá va a trabajar, vamos a ayudarlo,...}' //
\endgl
\xe

%
\end{document}